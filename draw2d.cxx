...
int main(int argc, char const *argv[]) {
    ...
    }
    {
        ...
        auto uplimit = config.value("max_x_value", 1.0);
        size_t i{};
        for (const auto &entry : config["hists"]) {
            ...
            entries.emplace_back(plot_name, std::move(hist));
            entries.emplace_back(plot_name + "_X", normalize_slice(hist_2dptr, 0));
            entries.emplace_back(plot_name + "_Y", normalize_slice(hist_2dptr, 1));
        }
        if (config.contains("misc")) {
            const std::unordered_map<std::string, std::function<std::unique_ptr<TObject>(const nlohmann::json &)>> misc_objects_maker{
                {"TLine",
                ...
                {"VLine",
                 [&max](const nlohmann::json &config) -> std::unique_ptr<TObject> {
                ...
                {"HLine",
                 [&uplimit](const nlohmann::json &config) -> std::unique_ptr<TObject> {
                ...
                {"TArrow", [](const nlohmann::json &config) -> std::unique_ptr<TObject> {
                     auto arrow = std::make_unique<TArrow>(config["x1"], config["y1"], config["x2"], config["y2"], config.value("arrowsize", 0.1),
...

    {
        for (std::size_t i = 0; i < entries.size(); ++i) {
            auto leg = config.contains("legend_place")
                           ? std::make_unique<TLegend>(config["legend_place"]["x1"], config["legend_place"]["y1"], config["legend_place"]["x2"],
                                                       config["legend_place"]["y2"], config.value("legend_header", "").c_str())
            ...
            for (const std::string &output_name : config["output_names"]) {
                auto real_name = legend_title + output_name;
                if (!std::filesystem::path(real_name).parent_path().empty())
                    std::filesystem::create_directories(std::filesystem::path(real_name).parent_path());
                canvas->SaveAs(real_name.c_str());
            }
        }
    }

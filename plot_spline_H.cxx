...
auto get_spline_nuwro(const std::vector<std::string> &channels,
  ...
  auto file =
      TFile::Open(("~/neutrino/nuwro_xsec/plots_lfg/H/" +
                   interaction.substr(0, interaction.length() - 3) + ".root")
                      .c_str(),
                  "READONLY");
  return TF1{
      interaction.c_str(),
...
int main(int argc, char **argv) {
  ...
  for (int realid = 0; auto &&[id, spline] : std::views::enumerate(splines)) {
    // spline.SetMinimum(0);
    ResetStyle(&spline);
    ...
    for (auto &&[id, spline] : std::views::enumerate(splines_nuwro)) {
      ...
      if (spline.Eval(10) != 0) {
        ...
        legend_NuWro->AddEntry(
            &spline, name_normalize(std::string{spline.GetName()}).c_str(),
            "l");
      }
    }
    legend_NuWro->Draw();
  ...
  //     spline.SetLineColor(colors[id]);
  //     spline.SetLineWidth(style[id] == kDashed ? 3 : 2);
  //     spline.GetXaxis()->SetLabelSize(textsize * fair_share / (1 -
  //     fair_share)); spline.GetXaxis()->SetTitleSize(textsize * fair_share /
  //     (1 - fair_share));

  //     spline.GetYaxis()->SetLabelSize(textsize * fair_share / (1 -
  //     fair_share)); spline.GetYaxis()->SetTitleSize(textsize * fair_share /
  //     (1 - fair_share)); spline.GetYaxis()->SetTitleOffset(titleoffset /
  //                                       (fair_share / (1 - fair_share)));
  //     if (spline.Eval(10) != 0)
  //       spline.Draw("same");
  //   }
